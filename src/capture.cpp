...
struct capture_object {
  ...
  cv::Mat gray_img_right; // YES shut the fuck up
...
capture_object *the_one_and_only;

// Functions

void hack_get_images(capture_object *obj) {
...
void fill_lut(capture_object *obj) {
  for (int y = 0; y < 21; y++) {
    for (int x = 0; x < 19; x++) {
      obj->uv_lut[y][x].x = (float)x / 18.0f;
      obj->uv_lut[y][x].y = (float)y / 20.0f;
      printf("%f %f\n", obj->uv_lut[y][x].x, obj->uv_lut[y][x].y);
    }
  }
};

void dummy_fill_wrap(capture_object *obj) {
...
draw_arucos_2(bool offsX, bool offsY, cv::Mat our_mat,
  ...
  for (int y = 0; y < 10 - 1; y++) {
    for (int x = 0; x < 9 - 1; x++) {
      ...
      cv::Rect rect(xoff_px + (x * 2 * side), yoff_px + (y * 2 * side), side,
                    side);
...
void debug_aruco_corner_order(cv::Mat img,
                              std::vector<std::vector<cv::Point2f>> corners,
                              std::vector<int> ids) {
  if (corners.size() == 0) {
    return;
  }
  cv::putText(img, "0", corners[0][0], cv::FONT_HERSHEY_SIMPLEX, 1,
              {255, 0, 0});
  cv::putText(img, "1", corners[0][1], cv::FONT_HERSHEY_SIMPLEX, 1,
              {255, 0, 0});
  cv::putText(img, "2", corners[0][2], cv::FONT_HERSHEY_SIMPLEX, 1,
              {255, 0, 0});
  cv::putText(img, "3", corners[0][3], cv::FONT_HERSHEY_SIMPLEX, 1,
              {255, 0, 0});
}

void debug_aruco_corners(
    cv::Mat img, std::vector<std::vector<cv::Point2f>> all_corners,
  ...
  for (int i = 0; i < ids.size(); i++) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      cv::putText(img, bob, all_corners[i][j], cv::FONT_HERSHEY_SIMPLEX, .5,
                  {255, 0, 0});
    }
  }
  // cv::putText(img, "0", corners[0][0], cv::FONT_HERSHEY_SIMPLEX, 1, {255, 0,
  // 0}); cv::putText(img, "1", corners[0][1], cv::FONT_HERSHEY_SIMPLEX, 1,
  // {255, 0, 0}); cv::putText(img, "2", corners[0][2],
  // cv::FONT_HERSHEY_SIMPLEX, 1, {255, 0, 0}); cv::putText(img, "3",
  // corners[0][3], cv::FONT_HERSHEY_SIMPLEX, 1, {255, 0, 0});
}

void setup_camera(capture_object *obj) {
  // Create pipeline
...
int main() {
  the_one_and_only = new capture_object;
  ...
  for (std::vector<int> fuck : fuckyou) {

    cv::Mat our_mat(1600, 1440 * 2, CV_8UC1);
    our_mat = cv::Scalar(255);

    draw_arucos_2(fuck[0], fuck[1], our_mat(cv::Rect(0, 0, 1440, 1600)),
                  our_dict);
    draw_arucos_2(fuck[0], fuck[1], our_mat(cv::Rect(1440, 0, 1440, 1600)),
                  our_dict);
    ...
    for (int i = 0; i < 16; i++) {
      ...
      char name[1288];
      sprintf(name, "captures/%d%d_%d_left.png", fuck[0], fuck[1], i);
      cv::imwrite(name, the_one_and_only->gray_img_left);

            sprintf(name, "captures/%d%d_%d_right.png", fuck[0], fuck[1], i);
